...
describe('CalculatorComponent', () => {
  ...
  it('should present results 5, 6 and 8, -1 and 1 after calculate(2,3)', () => {
    ...
    const subtractPowerInput:HTMLInputElement = compiled.querySelector('#resultSubtraction')!;
    const dividePowerInput:HTMLInputElement = compiled.querySelector('#resultDivision')!;
    expect(resultSumInput.value).toBe('5');
    expect(resultMultiplyInput.value).toBe('6');
    expect(resultPowerInput.value).toBe('8');
    expect(subtractPowerInput.value).toBe('-1');
    expect(dividePowerInput.value).toBe('1');
  });
  
  it('should present results 5, 6 and 8, -1 and 1 after user enter values 2 and 3 then click the button', () => {
    const compiled = fixture.nativeElement as HTMLElement;
